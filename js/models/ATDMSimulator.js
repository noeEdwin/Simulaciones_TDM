...
// ============================================================================
// EJEMPLO DE USO - Prueba con 5 canales para demostrar flexibilidad
// ============================================================================

// Ejemplo 1: Configuración con 5 canales y tramas de 3 slots
console.log('=== EJEMPLO 1: 5 Canales, Tramas de 3 Slots ===');
const sim1 = new ATDMSimulator(5, 3);
console.log(`Bits de dirección calculados: ${sim1.addressBits} bits`);

// Cargar datos
sim1.loadData('JAHIR,EDWIN,ALEXIS,MONTSE,VALVERDE');

// Simular proceso completo
console.log('\n--- Proceso de Multiplexación ---');
let frameCount = 0;
while (!sim1.isComplete()) {
    const frame = sim1.stepMux();
    if (frame) {
        frameCount++;
        console.log(`\nTrama ${frameCount}:`);
        frame.forEach((slot, i) => {
            console.log(`  Slot ${i}: [${slot.binaryAddress}|${slot.data}] (Canal ${slot.channelID})`);
        });
        
        // Demultiplexar
        sim1.stepDemux(frame);
    }
}

console.log('\n--- Resultados Finales ---');
console.log('Buffers de salida:', sim1.getOutputBuffers());
console.log('Estadísticas:', sim1.getStats());

// Ejemplo 2: Configuración con 8 canales (máximo típico)
console.log('\n\n=== EJEMPLO 2: 8 Canales, Tramas de 5 Slots ===');
const sim2 = new ATDMSimulator(8, 5);
console.log(`Bits de dirección calculados: ${sim2.addressBits} bits`);

sim2.loadData('A,B,C,D,E,F,G,H');
console.log('Buffers de entrada:', sim2.getInputBuffers());

// Procesar primera trama
const firstFrame = sim2.stepMux();
if (firstFrame) {
    console.log('\nPrimera Trama:');
    firstFrame.forEach(slot => {
        console.log(`  [${slot.binaryAddress}|${slot.data}] -> Canal ${slot.channelID}`);
    });
}

console.log('\nEstadísticas parciales:', sim2.getStats());
